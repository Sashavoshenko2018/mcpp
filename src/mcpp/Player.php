...
use mcpp\event\block\SignChangeEvent;
use mcpp\event\entity\EntityDamageByEntityEvent;
use mcpp\event\entity\EntityDamageEvent;
use mcpp\event\entity\EntityLevelChangeEvent;
use mcpp\event\entity\EntityRegainHealthEvent;
use mcpp\event\entity\EntityShootBowEvent;
use mcpp\event\entity\ProjectileLaunchEvent;
use mcpp\event\inventory\CraftItemEvent;
use mcpp\event\inventory\InventoryCloseEvent;
use mcpp\event\inventory\InventoryCreationEvent;
use mcpp\event\inventory\InventoryPickupArrowEvent;
use mcpp\event\inventory\InventoryPickupItemEvent;
use mcpp\event\player\PlayerAnimationEvent;
use mcpp\event\player\PlayerBedEnterEvent;
use mcpp\event\player\PlayerBedLeaveEvent;
use mcpp\event\player\PlayerChatEvent;
use mcpp\event\player\PlayerCommandPostprocessEvent;
use mcpp\event\player\PlayerCommandPreprocessEvent;
use mcpp\event\player\PlayerDeathEvent;
use mcpp\event\player\PlayerDropItemEvent;
use mcpp\event\player\PlayerGameModeChangeEvent;
use mcpp\event\player\PlayerInteractEvent;
use mcpp\event\player\PlayerItemConsumeEvent;
use mcpp\event\player\PlayerJoinEvent;
use mcpp\event\player\PlayerKickEvent;
use mcpp\event\player\PlayerLoginEvent;
use mcpp\event\player\PlayerMoveEvent;
use mcpp\event\player\PlayerPreLoginEvent;
use mcpp\event\player\PlayerQuitEvent;
use mcpp\event\player\PlayerReceiptsReceivedEvent;
use mcpp\event\player\PlayerRespawnAfterEvent;
use mcpp\event\player\PlayerRespawnEvent;
use mcpp\event\player\PlayerToggleSneakEvent;
use mcpp\event\player\PlayerToggleSprintEvent;
use mcpp\event\TextContainer;
use mcpp\form\Form;
use mcpp\inventory\BaseTransaction;
use mcpp\inventory\EnchantInventory;
use mcpp\inventory\Inventory;
use mcpp\inventory\InventoryHolder;
use mcpp\inventory\PlayerInventory;
use mcpp\inventory\ShapedRecipe;
use mcpp\inventory\ShapelessRecipe;
use mcpp\inventory\SimpleTransactionGroup;
use mcpp\inventory\transactions\SimpleTransactionData;
use mcpp\item\Armor;
use mcpp\item\Elytra;
use mcpp\item\enchantment\Enchantment;
use mcpp\item\Item;
use mcpp\item\Potion;
use mcpp\item\Tool;
...
use mcpp\metadata\MetadataValue;
use mcpp\nbt\NBT;
use mcpp\nbt\tag\Compound;
use mcpp\nbt\tag\DoubleTag;
use mcpp\nbt\tag\Enum;
...
use mcpp\player\PlayerSettingsTrait;
use mcpp\plugin\Plugin;
use mcpp\scheduler\CallbackTask;
use mcpp\scheduler\TaskHandler;
use mcpp\tile\Sign;
    ...
    public function handleDataPacket(DataPacket $packet)
    {
        ...
        switch($packet->pname()){
            ...
            case 'RESOURCE_PACKS_CLIENT_RESPONSE_PACKET':
                switch($packet->status){
                    ...
                    case ResourcePackClientResponsePacket::STATUS_SEND_PACKS:
                        $packManager = $this->server->getPackManager();
                        foreach($packet->packIds as $packId){
                            $resourcePack = $packManager->getResourcePackById($packId);
                    ...
                    case ResourcePackClientResponsePacket::STATUS_HAVE_ALL_PACKS:
                        $packManager = $this->server->getPackManager();
                        $pk = new ResourcePackStackPacket();
                        $pk->isRequired = $packManager->isPacksRequired();
                        $pk->addons = $packManager->getAddons();
                        $pk->resourcePacks = $packManager->getResourcePacks();
                        $this->dataPacket($pk);
                        break;
            ...
            case "RESOURCE_PACK_CHUNK_REQUEST_PACKET":
                $packManager = $this->server->getPackManager();
                $resourcePack = $packManager->getResourcePackById($packet->resourcePackId);
                if(is_null($resourcePack)){
