...
class CrashDump{
	...
	private function pluginsData(){
		if(class_exists("mcpp\\plugin\\PluginManager", false)){
			$this->addLine();
	...
	private function baseCrash(){
		...
		if(strpos($error["file"], "src/mcpp/") === false and strpos($error["file"], "src/raklib/") === false and file_exists($error["fullFile"])){
			$this->addLine();
			...
			foreach($this->server->getPluginManager()->getPlugins() as $plugin){
				$filePath = \mcpp\cleanPath($file->getValue($plugin));
	...
	private function generalData(){
		...
		$this->data["general"]["api"] = \mcpp\API_VERSION;
		$this->data["general"]["git"] = \mcpp\GIT_COMMIT;
