...
use mcpp\command\PluginIdentifiableCommand;
use mcpp\command\SimpleCommandMap;
use mcpp\entity\Arrow;
use mcpp\entity\Effect;
use mcpp\entity\Entity;
use mcpp\entity\ExperienceOrb;
use mcpp\entity\FallingSand;
use mcpp\entity\Human;
use mcpp\entity\Item as DroppedItem;
use mcpp\entity\Painting;
use mcpp\entity\PrimedTNT;
use mcpp\entity\projectile\BottleOEnchanting;
use mcpp\entity\projectile\SplashPotion;
use mcpp\entity\Snowball;
use mcpp\entity\Egg;
use mcpp\entity\Squid;
use mcpp\entity\Villager;
use mcpp\event\HandlerList;
use mcpp\event\level\LevelInitEvent;
use mcpp\event\level\LevelLoadEvent;
use mcpp\event\server\ServerCommandEvent;
use mcpp\event\Timings;
use mcpp\event\TimingsHandler;
use mcpp\inventory\CraftingManager;
use mcpp\inventory\InventoryType;
use mcpp\inventory\Recipe;
use mcpp\inventory\ShapedRecipe;
use mcpp\inventory\ShapelessRecipe;
use mcpp\inventory\FurnaceRecipe;
use mcpp\item\enchantment\Enchantment;
use mcpp\item\Item;
use mcpp\level\format\anvil\Anvil;
use mcpp\level\format\pmanvil\PMAnvil;
use mcpp\level\format\LevelProviderManager;
use mcpp\level\format\mcregion\McRegion;
use mcpp\level\generator\Generator;
use mcpp\level\Level;
use mcpp\metadata\EntityMetadataStore;
use mcpp\metadata\LevelMetadataStore;
use mcpp\metadata\PlayerMetadataStore;
use mcpp\mods\ModsManager;
use mcpp\nbt\NBT;
use mcpp\nbt\tag\ByteTag;
use mcpp\nbt\tag\Compound;
use mcpp\nbt\tag\DoubleTag;
...
use mcpp\scheduler\CallbackTask;
use mcpp\scheduler\GarbageCollectionTask;
use mcpp\scheduler\ServerScheduler;
use mcpp\tile\Bed;
use mcpp\tile\Cauldron;
use mcpp\tile\Chest;
use mcpp\tile\EnchantTable;
use mcpp\tile\EnderChest;
use mcpp\tile\Furnace;
use mcpp\tile\ItemFrame;
use mcpp\tile\Sign;
use mcpp\tile\Skull;
use mcpp\tile\FlowerPot;
use mcpp\tile\Tile;
use mcpp\utils\Binary;
use mcpp\utils\Cache;
use mcpp\utils\Config;
use mcpp\utils\LevelException;
use mcpp\utils\MainLogger;
use mcpp\utils\ServerException;
use mcpp\utils\TextFormat;
use mcpp\utils\TextWrapper;
use mcpp\utils\Utils;
use mcpp\utils\UUID;
use mcpp\utils\VersionString;
use mcpp\network\protocol\Info;
use mcpp\level\generator\biome\Biome;
use mcpp\scheduler\FileWriteTask;
use mcpp\entity\animal\walking\Chicken;
use mcpp\entity\animal\walking\Cow;
use mcpp\entity\animal\walking\Mooshroom;
use mcpp\entity\animal\walking\Ocelot;
use mcpp\entity\animal\walking\Pig;
use mcpp\entity\animal\walking\Rabbit;
use mcpp\entity\animal\walking\Sheep;
use mcpp\entity\monster\flying\Blaze;
use mcpp\entity\monster\flying\Ghast;
use mcpp\entity\monster\walking\CaveSpider;
use mcpp\entity\monster\walking\Creeper;
use mcpp\entity\monster\walking\Enderman;
use mcpp\entity\monster\walking\IronGolem;
use mcpp\entity\monster\walking\PigZombie;
use mcpp\entity\monster\walking\Silverfish;
use mcpp\entity\monster\walking\Skeleton;
use mcpp\entity\monster\walking\SnowGolem;
use mcpp\entity\monster\walking\Spider;
use mcpp\entity\monster\walking\Wolf;
use mcpp\entity\monster\walking\Zombie;
use mcpp\entity\monster\walking\ZombieVillager;
use mcpp\entity\projectile\FireBall;
use mcpp\utils\MetadataConvertor;
use mcpp\event\server\SendRecipiesList;
 ...
 */
class Server{
	const BROADCAST_CHANNEL_ADMINISTRATIVE = "mcpp.broadcast.admin";
	const BROADCAST_CHANNEL_USERS = "mcpp.broadcast.user";
	...
	public function saveOfflinePlayerData($name, Compound $nbtTag, $async = false){
		...
		try{
			...
			if(\mcpp\DEBUG > 1 and $this->logger instanceof MainLogger){
	...
	public function generateLevel($name, $seed = null, $options = []){
		...
		try{
			$path = $this->getDataPath() . "worlds/" . $name . "/";
			/** @var \mcpp\level\format\LevelProvider $provider */
	...
	public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $filePath, $dataPath, $pluginPath){	
		...
		if(!file_exists($this->dataPath . "mcpp-soft.yml")){
			$content = file_get_contents($this->filePath . "src/mcpp/resources/mcpp-soft.yml");
			@file_put_contents($this->dataPath . "mcpp-soft.yml", $content);
		}
		$this->softConfig = new Config($this->dataPath . "mcpp-soft.yml", Config::YAML, []);
		
		$this->logger->info("Loading mcpp.yml...");
		if(!file_exists($this->dataPath . "mcpp.yml")){
			$content = file_get_contents($this->filePath . "src/mcpp/resources/mcpp.yml");
			@file_put_contents($this->dataPath . "mcpp.yml", $content);
		}
		$this->config = new Config($this->dataPath . "mcpp.yml", Config::YAML, []);
		...
		define("mcpp\\DEBUG", (int) $this->getProperty("debug.level", 1));
		if($this->logger instanceof MainLogger){
			$this->logger->setLogDebug(\mcpp\DEBUG > 1);
		}

		Level::$COMPRESSION_LEVEL = $this->getProperty("chunk-sending.compression-level", 8);

		if(defined("mcpp\\DEBUG") and \mcpp\DEBUG >= 0){
...
//		$this->pluginManager->setUseTimings($this->getProperty("settings.enable-profiling", false));
	...
	public function start(){			
		...
		$this->logger->info("Done (" . round(microtime(true) - \mcpp\START_TIME, 3) . 's)! For help, type "help" or "?"');
	...
	private function titleTick(){
		if(defined("mcpp\\DEBUG") and \mcpp\DEBUG >= 0 and \mcpp\ANSI === true){
